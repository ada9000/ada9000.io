<script lang="ts">
	import { formatNumber, laceToAdaString, shortenStakeAddr } from '../../lib/util/utils';
	import { getDelegatorAssets, getDelegators } from 'data/koios';
	import cf from 'assets/cf.json';
	import type { DelegatorInfo } from 'types/types';
	import Adahandle from 'assets/icons/adahandle.svelte';
	// import { navigate } from 'svelte-navigator';
	import hoskyLogo from 'assets/images/hosky.png';
	let loading = true;
	...
	(async () => {
		delegators = await getDelegators().then((d) => {
			return d.sort((a, b) => b.lace - a.lace);
		});
		loading = false;
		delegators = await getDelegatorAssets(delegators).then((d) => {
			return d.sort((a, b) => b.lace - a.lace);
		});
		loadingAssets = false;
	})();
	...
	function laceWithoutCF() {
		const delegatorsWithoutCf = delegators.filter((x) => {
			return !cf.includes(x.stake_address);
		});
		let res = 0;
...
<div id="delegators">
	...
	{:else}
		...
		<div class="card py-4 shadow-lg max-w-sm mx-auto bg-base-300">
			...
			<p class="text text-center">active stake</p>

			<!-- if there is a CF whale notify user -->
			{#if delegators.filter((x) => {
				return cf.includes(x.stake_address);
			}).length > 0}
				<p class="text text-center text-xs">without Cardano Foundation whale included</p>
			{/if}
		</div>
		...
		<p class="text-center text-md my-4">
			<a
				...
				class="link link-hover">Free tokens ☂️</a
			>
		</p>

		<!-- Options -->
		<div class="card py-4 shadow-lg max-w-sm mx-auto bg-base-300 mt-4">
			<p class="text text-center">Options</p>

			<!-- Hosky toggle -->
			{#if loadingAssets}
				<div class="my-auto mx-auto mt-4 my-4 h-10">
					<progress class="progress mx-auto" />
					<p class="text text-sm text-center">Please wait loading assets</p>
				</div>
			{:else}
				<div class="w-12 my-auto mx-auto mt-4 my-4 h-10">
					<button on:click={handleHosky} disabled={loadingAssets}>
						{#if showHosky}
							<img
								src={hoskyLogo}
								alt="Hosky token logo. A pixel art of a Husky dog"
								class="hoskyOn"
							/>
						{:else}
							<img
	...
	<div class="grid gap-x-4 gap-y-2 xl:grid-cols-4 lg:grid-cols-3 sm:grid-cols-2 grid-cols-1">
		{#each delegators as delegator}
			{#if delegator.lace > 0}
				<button
					class="card mx-auto p-2 mt-4 shadow-lg max-w-sm bg-base-300 w-full items-center hover:bg-secondary group"
					on:click={() => {
				...
				>
					...
					{#if cf.includes(delegator.stake_address)}
						<p class="text text-center font-bold">🐋 Cardano Foundation</p>
					{:else if delegator.stake_address === 'stake1u8ag3gtdgtuj6hud7km4z0u0pcqvfndtq9fcckjvvc6yjdswaksu2'}
						<p class="text text-center font-bold">OWNER</p>
					{/if}
					...
					{#if delegator.handles && delegator.handles.length}
						<p class="text text-center text-lg font-bold group-hover:text-xl">
							<span class="inline-block h-4"><Adahandle /></span>
							{delegator.handles[0]}
						</p>
					{:else}
					...
					{#if showHosky && delegator.hosky}
						<div class="flex flex-col mx-auto my-auto p-2">
							...
							<p class="text text-center p-1">
								{formatNumber(delegator.hosky.toString())} Hosky
							</p>
						</div>
					{/if}
				</button>
