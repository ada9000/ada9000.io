  ...
  <div class="mx-auto content-center grid gap-2">
    ...
    {:then pool}
      ...
      <div id="stats" class="grid sm:grid-cols-3 cols-1 p-2 gap-x-6 gap-y-2">
        <div class="grid grid-rows-2 mx-auto gap-2">
          ...
          <div class="tooltip text-xl tooltip-secondary" data-tip="Pool operators fee. Allows 9000 to maintain a high quality operation.">
            <span class="badge badge-lg w-40">{(pool.margin * 100).toFixed(2)}%
              <InfoIcon/>
            </span>
          </div>
        </div>
        <div class="grid grid-rows-2 mx-auto gap-2">
          ...
          <div class="tooltip text-xl tooltip-secondary" data-tip="Amount charged if the pool mints a single block. 340 is current minimum allowed.">
            <span class="badge badge-lg w-40">{laceToAdaString(pool.fixed_cost.toString())}
              <InfoIcon/>
            </span>
          </div>
        </div>
        <div class="grid grid-rows-2 mx-auto gap-2">
          ...
          <div class="tooltip text-xl tooltip-secondary" data-tip="Amount of ada 9000 has pleged (aka skin in the game).">
            <span class="badge badge-lg w-40">{laceToAdaString(pool.pledge)}
              <InfoIcon/>
            </span>
          </div>
        </div>
    ...
    {:catch error}
      <div class="p-6 flex flex-col items-center">
        <p class="text text-lg text-center font-bold p-2">Sorry there was an issue fetching pool data</p>
