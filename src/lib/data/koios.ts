...
import type { AccountAsset, EpochInfo, PoolBlocks, PoolInfo, Tip } from '../types/koios';
...
export async function getEpoch(): Promise<EpochInfo> {
	...
	const epoch: EpochInfo = await fetch(`${ApiUrl}/epoch_info?_epoch_no=${tip.epoch_no}`).then(
		async (res) => {
			return res.json().then((x) => {
				return x[0];
			});
...
export async function getPool(poolBech32: string = PoolBech32): Promise<PoolInfo> {
...
export async function getDelegators(poolBech32: string = PoolBech32): Promise<DelegatorInfo[]> {
...
export async function getAccountAssets(addresses: string[]): Promise<AccountAsset[]> {
...
export async function getDelegatorAssets(delegators: DelegatorInfo[]): Promise<DelegatorInfo[]> {
	const updatedDelegators: DelegatorInfo[] = JSON.parse(JSON.stringify(delegators));
	...
	res.forEach((account) => {
		...
		const handlePolicy = 'f0ff48bbb7bbe9d59a40f1ce90e9e9d0ff5002ec48f232b49ca0fb9a';
		const hoskyPolicy = 'a0028f350aaabe0545fdcb56b039bfb08e4bb4d8c4d7c3c7d481c235';
